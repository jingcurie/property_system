...
<form action="{{ $formAction }}" method="POST" enctype="multipart/form-data" id="main-form">
    ...
    <!-- Application Info -->
    <div class="card mb-4">
        <div class="card-header fw-bold">{{ ut('modules.application.application_info') }}</div>
        <div class="card-body row g-3">
            @if (isset($property))
                {{-- 已知房源，自动填入且不可修改 --}}
                <div class="col-md-6">
                    <label class="form-label">{{ ut('modules.application.property') }}</label>
            ...
            @else
                {{-- 未知房源，显示下拉选择 --}}
                <div class="col-md-6">
                    <label for="property_id" class="form-label">{{ ut('modules.application.property') }} <span class="text-danger">*</span></label>
                    <select id="my-select" name="property_id"
                        ...
                        @if (!$selectedId)
                            <option value="">{{ ut('modules.application.select_property_placeholder') }}</option>
                        @endif
            ...
            <div class="col-md-6">
                <label for="application_code" class="form-label">{{ ut('modules.application.application_code') }} <span class="text-danger">*</span></label>
    ...
    <!-- Applicant Info -->
    <div class="card mb-4">
        <div class="card-header fw-bold">{{ ut('modules.application.applicant_info') }}</div>
        <div class="card-body row g-3">
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.full_name') }} <span class="text-danger">*</span></label>
            ...
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.email') }} <span class="text-danger">*</span></label>
                <input type="email" name="applicant[email]" class="form-control"
                    value="{{ old('applicant.email', $application->applicant->email ?? '') }}" required>
            </div>
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.phone') }} <span class="text-danger">*</span></label>
                <input type="text" name="applicant[phone]" class="form-control"
                    value="{{ old('applicant.phone', $application->applicant->phone ?? '') }}" required>
            </div>
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.date_of_birth') }} <span class="text-danger">*</span></label>
            ...
            <div class="col-md-4">
                <label class="form-label">{{ ut('modules.application.government_id_type') }}</label>
                <select name="applicant[government_id_type]" class="form-select">
                    @foreach (['SSN', 'SIN', 'ITIN'] as $opt)
                        <option value="{{ $opt }}"
                            ...
                            {{ ut('modules.application.id_types.' . $opt) }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
                <label class="form-label">{{ ut('modules.application.ssn_last4') }}</label>
                <input type="text" name="applicant[ssn_last4]" class="form-control"
                    value="{{ old('applicant.ssn_last4', $application->applicant->ssn_last4 ?? '') }}">
            </div>
            <div class="col-md-4">
                <label class="form-label">{{ ut('modules.application.address_line1') }} <span class="text-danger">*</span></label>
            ...
            <div class="col-md-4">
                <label class="form-label">{{ ut('modules.application.address_line2') }}</label>
            ...
            <div class="col-md-4">
                <label class="form-label">{{ ut('modules.application.city') }} <span class="text-danger">*</span></label>
                <input type="text" name="applicant[city]" class="form-control"
                    value="{{ old('applicant.city', $application->applicant->city ?? '') }}" required>
            </div>
            <div class="col-md-3">
                <label class="form-label">{{ ut('modules.application.state') }} <span class="text-danger">*</span></label>
                <input type="text" name="applicant[state]" class="form-control"
                    value="{{ old('applicant.state', $application->applicant->state ?? '') }}" required>
            </div>
            <div class="col-md-3">
                <label class="form-label">{{ ut('modules.application.zip_code') }} <span class="text-danger">*</span></label>
            ...
            <div class="col-md-3">
                <label class="form-label">{{ ut('modules.application.country') }} <span class="text-danger">*</span></label>
            ...
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.emergency_contact_name') }} <span class="text-danger">*</span></label>
            ...
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.emergency_contact_phone') }} <span class="text-danger">*</span></label>
            ...
            <div class="col-md-4">
                <label class="form-label">{{ ut('modules.application.insurance_provider') }}</label>
            ...
            <div class="col-md-4">
                <label class="form-label">{{ ut('modules.application.policy_number') }}</label>
            ...
            <div class="col-md-4">
                <label class="form-label">{{ ut('modules.application.coverage_amount') }}</label>
    ...
    <!-- Employment Info -->
    <div class="card mb-4">
        <div class="card-header fw-bold">{{ ut('modules.application.employment_income') }}</div>
        <div class="card-body row g-3">
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.employer_name') }} <span class="text-danger">*</span></label>
            ...
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.job_title') }} <span class="text-danger">*</span></label>
            ...
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.monthly_income') }} <span class="text-danger">*</span></label>
            ...
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.other_income_source') }}</label>
            ...
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.income_verified_by') }}</label>
                <select name="employment[income_verified_by]" class="form-select">
                    <option value="manual"
                        ...
                        {{ ut('modules.application.verification_methods.manual') }}
                    </option>
                    <option value="third_party"
                        {{ old('employment.income_verified_by', $application->employment->income_verified_by ?? '') == 'third_party' ? 'selected' : '' }}>
                        {{ ut('modules.application.verification_methods.third_party') }}
                    </option>
                </select>
            </div>
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.verification_date') }}</label>
    ...
    <!-- Consent Info -->
    <div class="card mb-4">
        <div class="card-header fw-bold">{{ ut('modules.application.authorization_consent') }}</div>
        <div class="card-body row g-3">
            <div class="col-md-6">
                <div class="form-check">
                    ...
                    <label class="form-check-label">{{ ut('modules.application.credit_check_consent') }}</label>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-check">
                    ...
                    <label class="form-check-label">{{ ut('modules.application.background_check_consent') }}</label>
                </div>
            </div>
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.esignature_provider') }}</label>
            ...
            <div class="col-md-6">
                <label class="form-label">{{ ut('modules.application.esignature_id') }}</label>
    ...
    <div class="form-check my-4">
        ...
        <label class="form-check-label" for="fair_housing_acknowledged">
            {{ ut('modules.application.fair_housing_policy') }}
        </label>
    ...
    <div class="text-end d-flex justify-content-between align-items-center">
        <a href="{{ route('rental_applications.index') }}" class="btn btn-outline-secondary">{{ ut('modules.application.cancel') }}</a>
        <button type="submit" class="btn btn-primary" onclick="submitMainForm()">
            {{ $isEdit ? ut('modules.application.update_application') : ut('modules.application.submit_application') }}
        </button>
    </div>
</form>
...
@push('scripts')
    <script>
        $(function() {
            $('#my-select').select2({
                tags: true,
                placeholder: '{{ ut('modules.application.select_or_input_placeholder') }}',
                allowClear: true
            });
        });
    </script>
