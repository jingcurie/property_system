...
@php $isAdmin = $user->name === "admin"; @endphp
    ...
    <div class="container py-4 d-flex justify-content-center">
        <div class="card user-form-card shadow-sm">
            <div class="card-header bg-primary text-white">
                ...
                @if ($isAdmin)
                    <div class="alert alert-warning small mt-2 mb-1">
                        当前用户是 <strong>超级管理员</strong>，仅允许修改头像和密码。
                    </div>
                @endif
            </div>
            <div class="card-body">
                <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        @if ($isAdmin)
                            <input type="text" name="role" class="form-control" value={{$user->getRoleNames()->first()}} readonly>
                        @else
                    ...
                    <div class="mb-3">
                        ...
                        <button type="button" class="btn btn-outline-primary ms-3" data-bs-toggle="modal"
                            data-bs-target="#avatarModal">选择头像</button>
                    </div>
    ...
    <script>
        document.querySelectorAll('.selectable-avatar').forEach(img => {
            img.addEventListener('click', function() {
                document.querySelectorAll('.selectable-avatar').forEach(i => i.classList.remove(
                'selected'));
