@if ($filter === 'status')
    <div class="filter-box flex-shrink-0 d-inline-block me-2 mb-2" style="min-width: 250px; max-width: 300px;"
        ...
        <select name="filter_values[status]" class="form-select">
            @foreach (['Available', 'Leased', 'Under Maintenance'] as $option)
    ...
    <div class="filter-box flex-shrink-0 d-inline-block me-2 mb-2" style="min-width: 250px; max-width: 300px;"
        ...
        <div class="d-flex gap-2">
            <div class="flex-fill">
                ...
                <input type="number" class="form-control" name="filter_values[rent][min]"
                    value="{{ $value['min'] ?? '' }}">
            </div>
            <div class="flex-fill">
                <label class="form-label">最大租金</label>
                <input type="number" class="form-control" name="filter_values[rent][max]"
                    value="{{ $value['max'] ?? '' }}">
            </div>
        </div>
        <button type="button" class="remove-filter">×</button>
    ...
    <div class="filter-box flex-shrink-0 d-inline-block me-2 mb-2" style="min-width: 250px; max-width: 300px;"
        ...
        <select name="filter_values[owner_id]" class="form-select">
            @foreach ($owners as $owner)
                <option value="{{ $owner->owner_id }}" @selected(($value ?? '') == $owner->owner_id)> {{ $owner->first_name }}
