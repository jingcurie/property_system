...
<form method="POST"
    ...
    <div class="container-fluid">
                ...
                <div class="col-md-6">
                    ...
                    <select name="property_type" class="form-select select-filter" required>
                        @foreach (['Apartment', 'House', 'Townhouse', 'Condo', 'Basement', 'Other'] as $type)
                            <option value="{{ $type }}" @selected(old('property_type', $property->property_type ?? '') == $type)> {{ $type }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="ownership_type" class="form-select select-filter" required>
                        @foreach (['Owned', 'Managed'] as $type)
                            <option value="{{ $type }}" @selected(old('ownership_type', $property->ownership_type ?? '') == $type)> {{ $type }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="address_province" class="form-select select-filter" required>
                        @foreach (['AB', 'BC', 'MB', 'NB', 'NL', 'NS', 'ON', 'PE', 'QC', 'SK'] as $province)
                            <option value="{{ $province }}" @selected(old('address_province', $property->address_province ?? '') == $province)> {{ $province }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <select name="parking_type" class="form-select select-filter">
                        @foreach (['Indoor', 'Outdoor', 'Garage', 'None'] as $type)
                            <option value="{{ $type }}" @selected(old('parking_type', $property->feature->parking_type ?? '') == $type)> {{ $type }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <select name="laundry" class="form-select select-filter" required>
                        @foreach (['In-unit', 'Shared', 'None'] as $option)
                            <option value="{{ $option }}" @selected(old('laundry', $property->feature->laundry ?? 'None') == $option)> {{ $option }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select name="availability_status" class="form-select select-filter" required>
                        @foreach (['Available', 'Leased', 'Under Maintenance'] as $status)
                            <option value="{{ $status }}" @selected(old('availability_status', $property->rentalInfo->availability_status ?? '') == $status)> {{ $status }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select name="lease_term_type" class="form-select select-filter" required>
                        @foreach (['Monthly', 'Fixed Term', 'Annual'] as $type)
                            <option value="{{ $type }}" @selected(old('lease_term_type', $property->rentalInfo->lease_term_type ?? '') == $type)> {{ $type }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <select name="pet_policy" class="form-select select-filter" required>
                        @foreach (['Allowed', 'Restricted', 'Not Allowed'] as $policy)
                            <option value="{{ $policy }}" @selected(old('pet_policy', $property->rentalInfo->pet_policy ?? '') == $policy)> {{ $policy }}
                            </option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="text-end">
            <button type="submit"
                class="btn btn-primary px-4 py-2 fs-7">{{ isset($property) ? '更新房源' : '保存房源' }}</button>
